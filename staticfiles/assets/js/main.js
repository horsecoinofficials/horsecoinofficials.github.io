...
(function() {
  ...
  async function fetchAndDisplayProducts() {
    try {
      ...
      const bestSellersContainer = document.querySelector('#best-sellers .row.g-5');
      const trendingNowContainer = document.querySelector('#cards .col-lg-4:nth-child(1) .product-list');
      const bestSellersCardsContainer = document.querySelector('#cards .col-lg-4:nth-child(2) .product-list');
      const featuredItemsContainer = document.querySelector('#cards .col-lg-4:nth-child(3) .product-list');

      if (bestSellersContainer) {
        bestSellersContainer.innerHTML = ''; // Clear existing static content
        products.filter(p => p.is_bestseller).slice(0, 4).forEach(product => {
          bestSellersContainer.innerHTML += createProductCardHtml(product, 'col-lg-3 col-md-6');
        });
      }

      if (trendingNowContainer) {
        trendingNowContainer.innerHTML = '';
        products.slice(0, 3).forEach(product => { // Display first 3 products as trending
          trendingNowContainer.innerHTML += createSmallProductCardHtml(product, 'badge-new');
        });
      }

      if (bestSellersCardsContainer) {
        bestSellersCardsContainer.innerHTML = '';
        products.filter(p => p.is_bestseller).slice(0, 3).forEach(product => { // Display first 3 bestsellers
          bestSellersCardsContainer.innerHTML += createSmallProductCardHtml(product, 'badge-sale', '-15%');
        });
      }

      if (featuredItemsContainer) {
        featuredItemsContainer.innerHTML = '';
        products.slice(3, 6).forEach(product => { // Display next 3 products as featured
          featuredItemsContainer.innerHTML += createSmallProductCardHtml(product, 'badge-limited');
        });
      }
  ...
  function createProductCardHtml(product, colClass) {
    const oldPriceHtml = product.old_price ? `<span class="old-price">$${product.old_price}</span>` : '';
    const currentPriceHtml = `<span class="current-price">$${product.price}</span>`;
    const badgeHtml = product.is_bestseller ? `<div class="product-badge">Best Seller</div>` : '';
    const discountBadgeHtml = product.discount_percentage ? `<div class="product-badge sale-badge">${product.discount_percentage}% Off</div>` : '';
    const ratingStars = generateStarHtml(parseFloat(product.rating));
    
    // Fix image path - ensure it has the correct format
    let imagePath = product.image;
    // First, check if the path already contains 'assets/img/products/' and replace it
    if (imagePath.includes('assets/img/products/')) {
        // Extract just the filename (e.g., product-1.svg)
      ...
      <div class="${colClass}">
        <div class="product-item">
          <div class="product-image">
            ${badgeHtml}
            ${discountBadgeHtml}
            <img src="/static/${imagePath}" alt="${product.name}" class="img-fluid" loading="lazy">
  ...
  function createSmallProductCardHtml(product, badgeClass = '', badgeText = '') {
    const ratingStars = generateStarHtml(parseFloat(product.rating));
    const badgeHtml = badgeClass ? `<div class="product-badges"><span class="${badgeClass}">${badgeText || product.category}</span></div>` : '';
    const oldPriceHtml = product.old_price ? `<span class="old-price">$${product.old_price}</span>` : '';
    const currentPriceHtml = `<span class="current-price">$${product.price}</span>`;
    ...
    return `
      <div class="product-card">
        <div class="product-image">
          <img src="/static/${imagePath}" alt="${product.name}" class="img-fluid">
          ${badgeHtml}
        </div>
        <div class="product-info">
